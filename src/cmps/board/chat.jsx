...
export const Chat = ({setIsChatOpen}) => {
    ...
    const onAddMassage = (ev) => {
        ...
        if (board.massages) board = { ...board, massages: [...board.massages, massageForBoard] }
        else board = { ...board, massages: [massageForBoard] }
        dispatch(updateBoard(board))
        setMassage('')
        const elMassages = document.getElementsByClassName('massage-list')[0]
        console.log(elMassages.scrollTop, elMassages.scrollHeight);
    ...
    return <section className="chat">
        ...
        <section className="user">
            ...
            <form onSubmit={onAddMassage}>
                <input value={massage} onChange={({ target }) => setMassage(target.value)} />
            </form>
        </section>
    </section>
}